...
export function toCodeStatusMap(
...
export function calcProblemCountByRank(problems: Problem[]) {
  ...
  for (const problem of problems) {
    ...
    if (preProblemRank === "1") {
      preProblemRank = "a";
    } else if (preProblemRank === "2") {
      preProblemRank = "b";
    } else if (preProblemRank === "3") {
      preProblemRank = "c";
...
export function calculateRank(
  ...
  if (status <= border.rank_a) {
    return "A";
  }
  if (status <= border.rank_b) {
    return "B";
  }
  if (status <= border.rank_c) {
    return "C";
  }
  if (status <= border.rank_d) {
    return "D";
  }
  return "E";
}
...
export function quantile(sortedArray: number[], percentile: number) {
  ...
  const result =
    sortedArray[i] + (sortedArray[i + 1] - sortedArray[i]) * fraction;
  return result;
}
