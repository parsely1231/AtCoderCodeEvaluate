...
function getQuantileIndex(
  codeSizeMedian: number,
  descSortedQuantiles: number[]
): number {
  for (const [index, quantile] of descSortedQuantiles.entries()) {
    if (codeSizeMedian >= quantile) {
      return index;
    }
  }
  return 7;
...
export const CodeSizeSquare: React.FC<CodeSizeSquareProps> = ({
  medianBorder,
  showCodeSize,
  quantiles,
  lengthUserRank
}) => {
  if (showCodeSize == false) {
    return null;
  }
  ...
  const fill: number =
    medianBorder >= quantiles[0]
      ...
      : 100 -
        Math.floor(
          ((medianBorder - quantiles[quantileIndex]) / borderRange) * 100
        );
  ...
  return (
    <div className="status-box">
      <Tooltip title={medianBorder}>
        <span className="length-square" style={styleOptions} />
      </Tooltip>
