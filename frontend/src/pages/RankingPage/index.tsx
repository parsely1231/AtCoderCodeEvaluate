...
const InnerRankingPage: React.FC<InnerProps> = ({
  rankingFetch,
  userId,
  language
}) => {
  const [orderBy, setOrderBy]: [RankingOrderBy, Function] = React.useState(
    "exec_time_points"
  );
  ...
  const rankingEntries = rankingFetch.fulfilled ? rankingFetch.value : [];
...
export const RankingPage = connect<OuterProps, InnerProps>(
  ({ userId, language }) => ({
    rankingFetch: {
      comparison: [userId, language],
      value: (): Promise<Required<RankingEntry>[]> => cachedRankings(language)
    }
  })
)(InnerRankingPage);
